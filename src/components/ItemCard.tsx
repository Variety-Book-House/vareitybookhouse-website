...
const ItemCard = ({ book, className, show = false }: ItemCardProps) => {
    ...
    return (
        ...
        `}
           ...
           ${show ? 'h-[100px]' : 'h-[80px] sm:h-[100px] lg:h-[100px]'}`}
            >
                <div>
                    <h3 className={`${show ? 'text-[14px]' : 'text-[12px] sm:text-[12px] lg:text-[14px]'}`}>
                        {title.toUpperCase()}
                    </h3>
                    <p className={`uppercase tracking-widest text-neutral-500 font-light line-clamp-1 ${show ? 'text-[12px]' : 'text-[10px] sm:text-[10px] lg:text-[12px]'}`}>
                        {author}
                    </p>
